...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Perfil privado para o modo Tor (não guarda cache/cookies no disco)
        ...
        # Abre a página do Tor por defeito para facilitar o teste
    ...
    def handle_tor_check_result(self, is_available):
        if is_available:
            # Força o Proxy Global
            proxy = QNetworkProxy(QNetworkProxy.ProxyType.Socks5Proxy, "127.0.0.1", 9050)
            QNetworkProxy.setApplicationProxy(proxy)
            
            # Limpa o cache para evitar que páginas reabram via rede normal armazenada
            self.private_profile.clearHttpCache()
            
            # Substitui as abas abertas por novas abas usando o perfil privado
                ...
                if splitter:
                    for i in range(splitter.count()):
                        ...
                        if isinstance(browser, QWebEngineView):
                            ...
                            # Cria uma nova página no perfil selecionado
                            new_page = QWebEnginePage(profile, browser)
                            browser.setPage(new_page)
                            
                            # Re-conecta os sinais essenciais à nova página/browser
